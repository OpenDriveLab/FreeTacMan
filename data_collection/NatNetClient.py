...
def trace_mf( *args):
    ...
    # print("pos".join(map(str, args)))
    # if "pos" in output:
    #     pos_data.append(output)
    #     txt_filename = f"{task_name}_{suffix}.txt"
...
class NatNetClient:
    ...
    def __init__( self , task_name, suffix):
    ...
    def __unpack_labeled_marker_data( self, data, packet_size, major, minor):
        ...
        if( ( major == 2 and minor > 3 ) or major > 2 ):
            ...
            for lm_num in range( 0, labeled_marker_count):
                ...
                trace_mf("    pos  : [%3.6f, %3.6f, %3.6f]"%(pos[0],pos[1],pos[2]))
                output_marker_id = f"{marker_id:03d}"
                output = f"pos: [{pos[0]:.6f}, {pos[1]:.6f}, {pos[2]:.6f}]"
                pos_data.append(output)
                import os
                save_dir = os.path.join('data', self.task_name)
                os.makedirs(save_dir, exist_ok=True)

                
                txt_filename = os.path.join(save_dir, f"{self.task_name}_{self.suffix}.txt")
                with open(txt_filename, "a") as file:
                    # file.write(time.strftime("%Y-%m-%d %H:%M:%S") + "\n")
                    file.write(str(float(time.time())) + "\n")
                    file.write(output_marker_id + "\n")
                    file.write(output + "\n")
                # print(output)
