...
def modified_resnet18(weights=None, features_per_group=16) -> nn.Module:
    ...
    # TODO: whether to use pretrained weights
...
class ClipDataset(torch.utils.data.Dataset):
            ...
            for episode_id in self.episode_ids:
                ...
                with h5py.File(dataset_path, 'r') as root:
                    ...
                    for timestep in range(self.episode_lengths[episode_id]):
                        ...
                        # resize tactile data
                        if self.tactile_size != tactile_data.shape[:2]:
                            raise ValueError("Image size must be the same for all cameras")
                            tactile_data = cv2.resize(tactile_data, (self.tactile_size[1], self.tactile_size[0]))
    ...
    def get_tactile(self, episode_idx, timestep):
        # get tactile data from the hdf5 file and preprocess it.
        dataset_path = os.path.join(self.dataset_dir, f'episode_{episode_idx}.hdf5')
        with h5py.File(dataset_path, 'r') as root:
            tactile_data = root['observations/images/tactile'][timestep]
...
def clip_loss_old(image_embeddings, tactile_embeddings, target_matrix, logit_scale = 1.0, visualize = False):
    ...
    for batch_idx in range(image_embeddings.shape[0]):
        ...
        tactile_targets = target_matrix.T
        ...
        for i in range(n_cameras):
            ...
            tactile_logits = logit_scale * tactile_embeddings[batch_idx] @ image_embeddings[batch_idx, :, i].T
            ...
            if visualize and batch_idx == 0:
                ...
                # tactile_softmax = F.softmax(tactile_logits.clone().detach(), dim=1).cpu().numpy()
                # visualizations.append((image_softmax + tactile_softmax)/2.0)

            image_loss = F.cross_entropy(image_logits, tactile_targets)
            tactile_loss = F.cross_entropy(tactile_logits, image_targets)
...
# main function for clip style pretraining
def clip_pretraining(
    ...
    # TODO: position dim, 6 here for joint state, not include gripper action
    state_size = 7
    ...
    # get a resnet18 model for tactile
    ...
    # TODO: remove the params you don't want to train
...
if __name__ == "__main__":
    ...
    args_parser.add_argument('--dataset_dir', type=str, default='/cpfs01/shared/opendrivelab/yuchecheng/whole_dataset_freetacman_1')
    args_parser.add_argument('--save_dir', type=str, default='/cpfs01/user/yuchecheng/FastUMI_Data/cobot_magic/aloha-devel/checkpoint/clip_resnet')
