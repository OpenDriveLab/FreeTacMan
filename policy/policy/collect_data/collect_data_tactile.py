...
def save_data(args, timesteps, actions, dataset_path):
    ...
    data_dict = {
        ...
        '/action': [],
        # '/base_action': [],
        # '/observations/left_endpose': [],
        # '/observations/right_endpose': [],
        # '/base_action_t265': [],
    }
    ...
    while actions:
        ...
        # data_dict['/observations/left_endpose'].append(ts.observation['left_endpose'])
        # data_dict['/observations/right_endpose'].append(ts.observation['right_endpose'])
    ...
    with h5py.File(dataset_path + '.hdf5', 'w', rdcc_nbytes=1024**2*2) as root:
        ...
        # TODO: 确认图片大小是否为640 480
        for cam_name in args.camera_names:
            _ = image.create_dataset(cam_name, (data_size, 640, 480, 3), dtype='uint8',
                                         chunks=(1, 640, 480, 3), )
        if args.use_depth_image:
            image_depth = obs.create_group('images_depth')
            for cam_name in args.camera_names:
                _ = image_depth.create_dataset(cam_name, (data_size, 640, 480), dtype='uint16',
                                             chunks=(1, 640, 480), )

        if len(args.arm_names) == 2:
            rob_state = 14
        elif len(args.arm_names) == 1:
            rob_state = 7
        _ = obs.create_dataset('qpos', (data_size, rob_state))
        # _ = obs.create_dataset('qvel', (data_size, rob_state))
        # _ = obs.create_dataset('effort', (data_size, rob_state))
        _ = root.create_dataset('action', (data_size, rob_state))
        # _ = obs.create_dataset('left_endpose', (data_size, 7))
        # _ = obs.create_dataset('right_endpose', (data_size, 7))
...
class RosOperator:
    def __init__(self, args):
        ...
        self.puppet_arm_left_endpose_deque = None
        self.puppet_arm_right_endpose_deque = None
    ...
    def get_frame(self):
        ...
        # print(len(self.puppet_arm_left_endpose_deque))
        # print(len(self.puppet_arm_left_endpose_deque))
        # print(self.puppet_arm_left_endpose_deque[-1].header.stamp.to_sec(), frame_time)
        ...
        # return (puppet_arm_left_endpose,puppet_arm_right_endpose)
        return (img_front, img_left, img_right, img_front_depth, img_left_depth, img_right_depth,
                puppet_arm_left, puppet_arm_right, master_arm_left, master_arm_right, robot_base, puppet_arm_left_endpose, puppet_arm_right_endpose)
    ...
    def init_ros(self):
        ...
        rospy.Subscriber(self.args.puppet_arm_left_endpose, Pose, self.puppet_arm_left_endpose_callback, queue_size=1000, tcp_nodelay=True)
        rospy.Subscriber(self.args.puppet_arm_right_endpose, Pose, self.puppet_arm_right_endpose_callback, queue_size=1000, tcp_nodelay=True)
    ...
    def process(self):
        ...
        while (count < self.args.max_timesteps + 1) and not rospy.is_shutdown():
            ...
            (img_front, img_left, img_right, img_front_depth, img_left_depth, img_right_depth,
             puppet_arm_left, puppet_arm_right, master_arm_left, master_arm_right, robot_base,puppet_arm_left_endpose,puppet_arm_right_endpose) = result
            # 2.1 图像信息
            image_dict = dict()
            
            # TODO: 更改需要传出的相机数量，我们只需要两个（腕部，视触觉）
            ...
            elif "left" in self.args.arm_names:
            ...
            elif "right" in self.args.arm_names:
            ...
            obs['left_endpose'] = np.array([puppet_arm_left_endpose.position.x,puppet_arm_left_endpose.position.y,puppet_arm_left_endpose.position.z,puppet_arm_left_endpose.orientation.x,puppet_arm_left_endpose.orientation.y,puppet_arm_left_endpose.orientation.z,puppet_arm_left_endpose.orientation.w])
            obs['right_endpose'] = np.array([puppet_arm_right_endpose.position.x,puppet_arm_right_endpose.position.y,puppet_arm_right_endpose.position.z,puppet_arm_right_endpose.orientation.x,puppet_arm_right_endpose.orientation.y,puppet_arm_right_endpose.orientation.z,puppet_arm_right_endpose.orientation.w])
            ...
            if len(self.args.arm_names) == 2:
                ...
                action = np.array(puppet_arm_left.position)
            elif "right" in self.args.arm_names:
                action = np.array(puppet_arm_right.position)
...
def get_arguments():
    ...
    parser.add_argument('--arm_names', action='store', type=int, help='collect which arm data',  
    ...
    # topic name of eep
    parser.add_argument('--puppet_arm_left_endpose', action='store', type=str, help='pose of left end effector',
                        default='/end_pose_left', required=False)
    parser.add_argument('--puppet_arm_right_endpose', action='store', type=str, help='pose of right end effector',
