...
from policy.act.detr.main import build_ACT_model_and_optimizer, build_CNNMLP_model_and_optimizer, build_diffusion_model_and_optimizer
...
class DiffusionPolicy(nn.Module):
    def __init__(self, args_override):
        ...
        # TODO: store the backbone modelname
        self.model = model
    ...
    def __call__(self, image, depth_image, robot_state, actions=None, action_is_pad=None):
        ...
        # TODO: normalize the image/tactile if using pretrained backbone
        B = robot_state.shape[0]
        if actions is not None:
            self.model.train()
            actions = actions[:, :self.model.chunk_size]
            action_is_pad = action_is_pad[:, :self.model.chunk_size]
            ...
            joint_loss = (all_l2 * ~action_is_pad.unsqueeze(-1)).mean(dim=(0, 1))

            loss_dict = {}
            loss_dict['l2_loss'] = loss
            loss_dict['loss'] = loss
            for i in range(len(joint_loss)):
                loss_dict[f'joint_{i}_loss'] = joint_loss[i]
...
class ACTPolicy(nn.Module):
    ...
    def __call__(self, image, depth_image, tactile_image, robot_state, actions=None, action_is_pad=None):
        ...
        if actions is not None:  # training time
            ...
            a_hat, (mu, logvar) = self.model(image, depth_image, tactile_image, robot_state, actions, action_is_pad)
            ...
            a_hat, (_, _) = self.model(image, depth_image, tactile_image, robot_state)  # no action, sample from prior
            return a_hat
