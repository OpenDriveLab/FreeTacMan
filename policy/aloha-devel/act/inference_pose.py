...
def get_current_pose(piper:C_PiperInterface_V2):
    
    # TODO: read current pose and gripper action
    msg = piper.GetArmEndPoseMsgs()
    current_pose = np.array([msg.end_pose])
    gripper_action = piper.GetArmGripperMsgs()
    
    return np.concatenate([current_pose, [gripper_action]], axis=0)
...
def get_model_config(args):
    ...
    if args.policy_class == 'ACT':
        policy_config = {'lr': args.lr,
                        ...
                        #  'tactile_sensor_names': task_config['tactile_sensor_names'],
                        #  'use_tactile_sensor': args.use_tactile_sensor,
...
def get_tactile_image(observation, camera_names):
    curr_images = []
    for cam_name in camera_names:
        curr_images.append(observation["tactile_sensors"][cam_name])
        
    curr_images = np.stack(curr_images, axis=0)
    curr_images = torch.from_numpy(curr_images / 255.0).float().cuda().unsqueeze(0)
    return curr_images

def PosetoPoseAction(pose: Pose, joint: JointState):
    pose_action = np.array([0.0] * 7, dtype=float)
    pose_action[:3] = (
        pose.position.x,
        pose.position.y,
        pose.position.z,
    ...
    rot = R.from_quat(quant)
    euler = rot.as_euler('xyz', False)
    pose_action[3:6] = np.array(euler)
    pose_action[6] = joint.position[6]
...
def inference_process(args, config, ros_operator, policy, stats, t, pre_action):
    ...
    while True and not rospy.is_shutdown():
        ...
        (img_front, img_left, img_right, img_front_depth, img_left_depth, img_right_depth,
         puppet_arm_left, puppet_arm_right, puppet_arm_left_endpose, puppet_arm_right_endpose, robot_base) = result
        
        # img_front_rgb = cv2.cvtColor(img_front, cv2.COLOR_BGR2RGB)
        # cv2.imwrite("front_view.jpg", img_front_rgb)
        ...
        if args.use_depth_image:
            ...
            for cam_name in config['camera_names']:
                if cam_name == "cam_high":
                    image_dict[cam_name] = img_front_depth
                elif cam_name == "cam_left_wrist":
                    image_dict[cam_name] = img_left_depth
                elif cam_name == "cam_right_wrist":
                    image_dict[cam_name] = img_right_depth

            obs['images_depth'] = image_depth_dict
            
        # if args.use_tactile_sensor:
        #     image_tactile_dict = dict()
        #     # TODO: store the tactile image into the dict
        #     obs['tactile_sensors'] = image_tactile_dict
            
            
        if len(args.arm_names) == 2:
            left_pose_action = PosetoPoseAction(puppet_arm_left_endpose, puppet_arm_left)
            right_pose_action = PosetoPoseAction(puppet_arm_right_endpose, puppet_arm_right)

            obs['qpos'] = np.concatenate(
                (left_pose_action, right_pose_action), axis=0)
        elif "left" in args.arm_names:
            left_pose_action = PosetoPoseAction(puppet_arm_left_endpose, puppet_arm_left)
            obs['qpos'] = np.array(left_pose_action)
        elif "right" in args.arm_names:
            right_pose_action = PosetoPoseAction(puppet_arm_right_endpose, puppet_arm_right)
            obs['qpos'] = np.array(right_pose_action)
...
def model_inference(args, config, ros_operator, save_episode=True):
    ...
    right1 = [0.254228856, 1.530327232, -0.93927218, -0.057094212000000005, 0.855331652, 0.16999178, 0.0]
    ...
    with torch.inference_mode():
        while True and not rospy.is_shutdown():
            ...
            while t < max_publish_step and not rospy.is_shutdown():
                ...
                ros_operator.puppet_arm_endpose_publish(left_action, right_action)  # puppet_arm_publish_continuous_thread
                if args.use_robot_base:
...
class RosOperator:
    def __init__(self, args):
        ...
        self.puppet_arm_left_endpose_publisher = None
        self.puppet_arm_right_endpose_publisher = None
    ...
    def init(self):
        ...
        self.puppet_arm_left_endpose_deque = deque()
        self.puppet_arm_right_endpose_deque = deque()
        self.robot_base_deque = deque()
        self.puppet_arm_publish_lock = threading.Lock()
        self.puppet_arm_publish_lock.acquire()

    # TODO: publish eepose
    def puppet_arm_endpose_publish(self, left, right):
        pose_msg = Pose()
        ...
        if left.any():
            end_pose_msg.x, end_pose_msg.y, end_pose_msg.z = left[:3] * POS_FACTOR
            end_pose_msg.roll, end_pose_msg.pitch, end_pose_msg.yaw = left[3:6] * ROT_FACTOR
            end_pose_msg.gripper = left[6]
            self.puppet_arm_left_endpose_publisher.publish(end_pose_msg)
        
        if right.any():
            end_pose_msg.x, end_pose_msg.y, end_pose_msg.z = right[:3] * POS_FACTOR
            end_pose_msg.roll, end_pose_msg.pitch, end_pose_msg.yaw = right[3:6] * ROT_FACTOR
            end_pose_msg.gripper = right[6]
            self.puppet_arm_right_endpose_publisher.publish(end_pose_msg)
    ...
    def get_frame(self):
        ...
        if len(self.puppet_arm_right_deque) == 0 or self.puppet_arm_right_deque[-1].header.stamp.to_sec() < frame_time:
            return False
        if len(self.puppet_arm_left_endpose_deque) == 0 or self.puppet_arm_left_endpose_deque[-1].header.stamp.to_sec() < frame_time:
            return False
        if len(self.puppet_arm_right_endpose_deque) == 0 or self.puppet_arm_right_endpose_deque[-1].header.stamp.to_sec() < frame_time:
            return False
        ...
        while self.puppet_arm_left_endpose_deque[0].header.stamp.to_sec() < frame_time:
            self.puppet_arm_left_endpose_deque.popleft()
        puppet_arm_left_endpose = self.puppet_arm_left_endpose_deque.popleft()

        while self.puppet_arm_right_endpose_deque[0].header.stamp.to_sec() < frame_time:
            self.puppet_arm_right_endpose_deque.popleft()
        puppet_arm_right_endpose = self.puppet_arm_right_endpose_deque.popleft()
        ...
        return (img_front, img_left, img_right, img_front_depth, img_left_depth, img_right_depth,
                puppet_arm_left, puppet_arm_right, puppet_arm_left_endpose, puppet_arm_right_endpose, robot_base)
    ...
    def puppet_arm_left_endpose_callback(self, msg):
        if len(self.puppet_arm_left_endpose_deque) >= 2000:
            self.puppet_arm_left_endpose_deque.popleft()
        self.puppet_arm_left_endpose_deque.append(msg)

    def puppet_arm_right_endpose_callback(self, msg):
        if len(self.puppet_arm_right_endpose_deque) >= 2000:
            self.puppet_arm_right_endpose_deque.popleft()
        self.puppet_arm_right_endpose_deque.append(msg)
    ...
    def init_ros(self):
        ...
        rospy.Subscriber(self.args.puppet_arm_left_endpose_topic, Pose, self.puppet_arm_left_endpose_callback, queue_size=1000, tcp_nodelay=True)
        rospy.Subscriber(self.args.puppet_arm_right_endpose_topic, Pose, self.puppet_arm_right_endpose_callback, queue_size=1000, tcp_nodelay=True)
        rospy.Subscriber(self.args.robot_base_topic, Odometry, self.robot_base_callback, queue_size=1000, tcp_nodelay=True)
        
        self.puppet_arm_left_endpose_publisher = rospy.Publisher(self.args.puppet_arm_left_endpose_cmd_topic, PosCmd, queue_size=10)
        self.puppet_arm_right_endpose_publisher = rospy.Publisher(self.args.puppet_arm_right_endpose_cmd_topic, PosCmd, queue_size=10)
...
def get_arguments():
    ...
    parser.add_argument('--ckpt_name', action='store', type=str, help='ckpt_name', default='policy_last.ckpt', required=False)  # policy_epoch_6000_seed_0.ckpt / policy_best.ckpt
    ...
    parser.add_argument('--puppet_arm_left_endpose_topic', action='store', type=str, help='puppet_arm_left_endpose_topic',
                        default='/end_pose_left', required=False)
    parser.add_argument('--puppet_arm_right_endpose_topic', action='store', type=str, help='puppet_arm_right_endpose_topic',
                        default='/end_pose_right', required=False)
    parser.add_argument('--puppet_arm_left_endpose_cmd_topic', action='store', type=str, help='puppet_arm_left_endpose_cmd_topic',
                        default='pos_cmd_left', required=False)
    parser.add_argument('--puppet_arm_right_endpose_cmd_topic', action='store', type=str, help='puppet_arm_right_endpose_cmd_topic',
    ...
    parser.add_argument('--use_depth_image', action='store', type=bool, help='use_depth_image',
                        default=False, required=False)
    parser.add_argument('--use_tactile_sensor', action='store', type=bool, help='use_tactile_sensor',
                        default=False, required=False)
